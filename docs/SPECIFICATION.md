...
- Rules are cited by name, not by section number. Outside this document, prose that relies on a rule states the rule (the merge-then-sign rule, the fixtures policy, the frontmatter standard) and, where a pointer helps, names this document: the specification, docs/SPECIFICATION.md in the marketplace repository. Section numbers move between revisions and cannot be resolved by a reader inside an installed plugin or a repository README, so they appear only in this document's own cross-references. Citations of external standards (CF conventions, the upstream OKF specification) keep the section numbers those standards publish. The durable citations elsewhere are a concept path, a tracking issue, a commit, or a register row defined in the same repository as the text that cites it. The rule is measured, not policed by hand: `tools/check_prose.py` in the nasa-daac-knowledge repository flags a section-number citation, program bookkeeping and em or en dashes, and runs in every plugin gate.
...
Every bundle (and, in large bundles, every subdirectory) has a named steward team in CODEOWNERS; PRs auto-request it. The maintainer who holds a bundle is its steward, and a data provider's confirmation is additive: it raises a concept's trust tier (§5.6), never the bar a concept has to clear. Merge rules: one human review of any role for any concept, and `stable` needs exactly that; two human reviews of any role for high-severity gotchas and for any edit that changes severity, status, or an Uncertainty section, where a provider review is preferred and invited, not required. The review checklist (docs/steward-playbook.md): `sources` resolve and actually support the claims that cite them; severity is calibrated (high means silently wrong results); scope is minimal (one trap per concept); a reproduction or eval case exists where required; the `verified: {by: human:<id>, at}` event is added at approval (§5.6), never by the drafting process. Stewards earn authorship on the bundle's Zenodo releases; onboarding follows the playbook plus the ARSET train-the-trainer methods pattern.

**The ladder of involvement.** Provenance is a ladder, and a person at a data center may stand on any rung of it. **Consulted:** they answer a "confirm this concept" issue (the organization's confirm_concept template) with confirmed, a correction, or not my product; the maintainer records the event on their behalf with `role: provider` and the reply's URL as `source` (§5.6). **Reviewer:** they review knowledge pull requests for their products on GitHub. **Steward:** they join the bundle's steward team in CODEOWNERS and sign with `tools/sign.py`. Nothing above consulted is required of anyone, and no rung is required for a concept to be stable, a capability to be released or promoted, or a composite to exist. The knowledge digest (`tools/digest.py` in nasa-daac-knowledge renders `knowledge/<bundle>/DIGEST.md`: what the bundle claims about each product, with status, tier, evidence and a confirm link per claim) is what a person reads to decide what they could take on. Promotion of a planned repository needs a maintainer, sources on every claim, evals for high-severity gotchas and a named provider contact who has been invited, not a signature; a composite needs a maintainer plus a reviewer from each sphere it touches, not its own steward.
...
`status` on every concept is `draft` (unreviewed; consultable but voiced as unverified), `stable` (ready for consumption; the default when absent), or `deprecated` (kept for links and history; the OSP extension key `superseded_by` names the replacement). Trust lives outside status: a human review adds a `verified: {by: human:<id>, at}` event (independent checks append to the list). The event keeps the OKF shape and carries two OSP extension keys inside it: `role`, one of `maintainer`, `provider` or `community` (absent means `maintainer`), and, when the confirmation was given somewhere other than the commit itself, `source`, the URL of the issue reply, review comment or message where the person confirmed. Consumers derive the trust tier from the events, keyed on the `human:` prefix, and the role adds a tier: unverified (no event), machine-confirmed (process events only), human-reviewed (a `human:` event), provider-confirmed (a `human:` event with `role: provider`). Provider-confirmed is a badge the runtime voices, never a precondition. Staleness is a date comparison: `stale_after` carries the sweep date and a concept is stale once now >= stale_after; a product baseline change (for example, a SWOT processing-version bump) pulls the date up and triggers a steward sweep of that dataset's concepts. A dispute is the OSP extension `disputed: <open issue URL>` on a stable concept; skills MUST state the dispute when citing. Skills surface status and tier when citing: high-severity claims are voiced with their verification provenance.
